...
# =====================================
command = ['/function', '/submit', '/view', '/zhuce', '/admin_viewallid', '/admin_addid', '/admin_delid',
           '/admin_delcode', '/admin_viewcode', '/admin_reboot', '/admin_reset']
description = ['功能简介:', '提交功能', '查询上车和我的提交互助码', '注册上车权限限', '管理员查看所有id', '管理员处理审核加入id', '管理员处理投诉后删除ID', '管理员删除错误互助码',
               '管理员查询互助码', '管理员重启机器人', '管理员清空所有数据']
hd_nm = ['NC@京东农场', 'AS@签到领现金', 'MC@萌宠', 'JC@惊喜工厂', 'DC@京东工厂', 'ZD@种豆', 'MH@盲盒', 'JN@暂时无', 'GJ@京东国际']
hd_me = ['ME@注册圈友人数']
bot_zhuce = ['请注册后使用',
             '你好,很多群友反应加了互助码库，却没有跑互助码库脚本，只有他们给你助力，你却没有给他们助力，所以采用微信群友审核机制，他们审核同意后，联系公众号iosrule,群主加入权限.本人只提供机器人服务，其他不懂',
             '你已经上车', '你好，群友偶尔如有要求助力截图,请在群里发助力截图']
hd_codelist = []
hd_memlist = []
zhjbzhenda = []
bot_timeout = 15
...
tback = [
    '1.机器响应时间15-30秒,请在对话框输入字符 /  查看对应指令再发送内容。每个京东活动互助码分开提交,格式:活动简称大写字母+互助码,多个互助码用@连接,例如京东农场NC12333@885666@8556\n', ]
                ...
                if mlist[0] == '/submit':
                    ...
                    for ll in hd_nm:
                        ...
                        if mlist[1].find(ll[0:2]) == 0:
                            ...
                            postmsg = ll[3:len(ll)] + '活动共计提交' + str(len(postmsg.strip().split('@'))) + '个互助码,其中' + str(
                                allnum) + '个为有效互助码，其他为重复数据,1个小时后更新进数据库....'
                            break
                        else:
...
def tg_notice(x):
    if x == 1 and r == 2:
        bot_sendmsg(tg_admin_id, '净网行动提示:', '网警95327来了')
    elif x == 2 and r == 2:
        bot_sendmsg(tg_admin_id, '净网行动提示:', '网警95327暂时离开')
